...
def FCIstats (G, testfci):
    ...
    #SHD calculation
    #true_edges = set([(e.get_node1().get_name(), e.get_node2().get_name()) for e in G.get_graph_edges()])
    #pred_edges = set([(e.get_node1().get_name(), e.get_node2().get_name()) for e in testfci.get_graph_edges()])

    #sym_diff = true_edges ^ pred_edges
    #flipped = set([(edge[1], edge[0]) for edge in sym_diff])
    #intersect = sym_diff & flipped

    #fciStat.append(len(sym_diff) - len(intersect) / 2)
...
def CDNODstats(G, testcdnod):
    ...
    #SHD calculation
    true_edges = set([(e.get_node1().get_name(), e.get_node2().get_name()) for e in G.get_graph_edges()])
    pred_edges = set([(e.get_node1().get_name(), e.get_node2().get_name()) for e in testcdnod.get_graph_edges()])

    sym_diff = true_edges ^ pred_edges
    flipped = set([(edge[1], edge[0]) for edge in sym_diff])
    intersect = sym_diff & flipped

    cdnodStat.append(len(sym_diff) - len(intersect) / 2)
