...
class ArrowConfusion:
    ...
    def get_arrows_precision(self):
        den = (self.__arrowsTp + self.__arrowsFp)
        if den:
            return self.__arrowsTp / den
        return np.NaN

    def get_arrows_recall(self):
        den = (self.__arrowsTp + self.__arrowsFn)
        if den:
            return self.__arrowsTp / den
        return np.NaN

    def get_arrows_precision_ce(self):
        den = (self.__arrowsTpCE + self.__arrowsFpCE)
        if den:
            return self.__arrowsTpCE / den
        return np.NaN

    def get_arrows_recall_ce(self):
        den = (self.__arrowsTpCE + self.__arrowsFnCE)
        if den:
            return self.__arrowsTpCE / den
        return np.NaN

    def get_arrows_Mc(self):
        a = (self.__arrowsTp * self.__arrowsTn) - (self.__arrowsFp * self.__arrowsFn)
        b = (self.__arrowsTp + self.__arrowsFp) * (self.__arrowsTp + self.__arrowsFn) * (self.__arrowsTn + self.__arrowsFp) * (self.__arrowsTn + self.__arrowsFn)
        if b == 0:
            b = 1
        return a / np.sqrt(b)
    
    def get_arrows_F1(self):
        den1 = (self.__arrowsTp + self.__arrowsFp)
